...
- Note las diferencias entre trabajar con `Rigidbody` y `CharacterController`. Por ejemplo: que pasa con las pequeñas plataformas amarillas que hay en la escena? Como interactuan? Que pasa cuando el Player sale de las plataformas?
  Respuesta: Cuando se pasa sobre las plataformas amarillas con  `Rigidbody` la colisión es notable y solo cuando se camina en diagonal es posible subir las escaleras, con el `CharacterController` se siente más fluido, como si de verdad estuviera subiendo unas escaleras, pues hay un pequeño impulso notable cuando se pasa sobre ellas, cuando el player sale de las plataformas, con el `CharacterController` y `Rigidbody` se nota la caída de nivel.
**Reto (Opcional)**
