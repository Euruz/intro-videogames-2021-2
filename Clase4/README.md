...
- Note las diferencias entre trabajar con `Rigidbody` y `CharacterController`. Por ejemplo: que pasa con las pequeñas plataformas amarillas que hay en la escena? Como interactuan? Que pasa cuando el Player sale de las plataformas?
    - La principal diferencia entre ambos componentes a la hora de mover el personaje es que uno recive como parámetro la velocidad a la cuál debe mover el personaje para cada dirección con un vector 3, mientras que el segundo ( CharacterController ) recibe como parámetro los valores que se agregarán a la posición del personaje, por lo tanto requiere realizar antes una multiplicación por Time.deltaTime para trabajar con la velocidad de movimiento deseada. Por otro lado, debido a que el CharacterController no maneja la gravedad del personaje, cuando éste sube a las plataformas conserva su altura luego de salir de ellas, sin embargo agregué una pequeña modificación al script que permite que el personaje baje, generando "gravedad", haciendo que el personaje trate de moverse hacia abajo constanemente. 
